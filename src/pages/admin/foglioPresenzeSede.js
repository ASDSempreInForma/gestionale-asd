import { PDFDocument, StandardFonts, rgb } from "pdf-lib";

/* =====================================================================
   FOGLIO PRESENZE SEDE (PDF) — A.S.D. Sempre In Forma
   Replica la struttura del foglio Excel "Elenco Corso" usato finora per
   i turni SEDE: intestazione con logo, Anno/Corso/Orario/N.Lezioni, poi
   una riga per ogni data di lezione con una colonna per ogni iscritto
   (spazio per firma/presenza) e una colonna Firma istruttore.
   Le date delle lezioni si calcolano da una data di inizio scelta
   dall'admin, una a settimana nel giorno del turno, saltando i periodi
   di sospensione indicati (richiesto da Solomon il 13/09/2026).
   ===================================================================== */

const LOGO_ASD_B64 = "";

function faseNumeroASettimana(giorno) { return giorno; } // 1=Lunedì ... 6=Sabato (come sede_turni)

function calcolaDateLezioni(dataInizioISO, giornoSettimana, nLezioni, esclusioni) {
  const date = [];
  let cursor = new Date(dataInizioISO + "T00:00:00");
  // porta il cursore al primo giorno della settimana corretto, senza tornare indietro
  const diff = (giornoSettimana - cursor.getDay() + 7) % 7;
  cursor.setDate(cursor.getDate() + diff);

  let guardia = 0;
  while (date.length < nLezioni && guardia < 300) {
    guardia++;
    const d = cursor.toISOString().slice(0, 10);
    const esclusa = (esclusioni || []).some((e) => e.dal && e.al && d >= e.dal && d <= e.al);
    if (!esclusa) date.push(new Date(cursor));
    cursor.setDate(cursor.getDate() + 7);
  }
  return date;
}

function stagioneDa(dataISO) {
  const d = new Date(dataISO + "T00:00:00");
  const anno = d.getMonth() >= 7 ? d.getFullYear() : d.getFullYear() - 1; // da agosto in poi è la nuova stagione
  return `${anno}/${String(anno + 1).slice(2)}`;
}

const GIORNI_LABEL_FP = ["Domenica", "Lunedì", "Martedì", "Mercoledì", "Giovedì", "Venerdì", "Sabato"];

export async function generaFoglioPresenzeSede(turno, iscritti, dataInizioISO, esclusioni) {
  const matchLezioni = String(turno.note || "").match(/N\.Lezioni:(\d+)/);
  const nLezioni = matchLezioni ? Number(matchLezioni[1]) : 8;
  const dateLezioni = calcolaDateLezioni(dataInizioISO, turno.giorno_settimana, nLezioni, esclusioni);

  const pdfDoc = await PDFDocument.create();
  const fontBold = await pdfDoc.embedFont(StandardFonts.HelveticaBold);
  const fontRegular = await pdfDoc.embedFont(StandardFonts.Helvetica);
  const logoImage = await pdfDoc.embedJpg(Uint8Array.from(atob(LOGO_ASD_B64), (c) => c.charCodeAt(0)));
  const logoDims = logoImage.scale(0.35);

  const W = 841.89, H = 595.28; // A4 orizzontale in punti
  const MARGINE = 30;
  const nero = rgb(0.1, 0.1, 0.1);
  const grigio = rgb(0.5, 0.5, 0.5);

  const persone = iscritti || [];
  const nomeCol = (p) => `${p.cognome || ""} ${p.nome || ""}`.trim();

  const colFisse = [
    { label: "N°", w: 26 },
    { label: "Data lezione", w: 78 },
  ];
  const colFirma = { label: "Firma istruttore", w: 90 };
  const larghezzaDisponibile = W - 2 * MARGINE - colFisse.reduce((s, c) => s + c.w, 0) - colFirma.w;
  const wPersona = persone.length > 0 ? Math.min(90, larghezzaDisponibile / persone.length) : 0;

  let page = pdfDoc.addPage([W, H]);
  const rigaAltezza = 24;
  const headerAltezza = 34;

  function disegnaIntestazionePagina(p) {
    p.drawImage(logoImage, { x: MARGINE, y: H - MARGINE - logoDims.height, width: logoDims.width, height: logoDims.height });
    const testoX = MARGINE + logoDims.width + 16;
    p.drawText("TESSERATI corsi SEDE", { x: testoX, y: H - MARGINE - 14, size: 15, font: fontBold, color: nero });
    const riga2 = `Anno: ${stagioneDa(dataInizioISO)}    Corso: PILATES    Orario: ${GIORNI_LABEL_FP[turno.giorno_settimana]} ${String(turno.orario || "").slice(0, 5)}    N. Lezioni: ${nLezioni}`;
    p.drawText(riga2, { x: testoX, y: H - MARGINE - 32, size: 10, font: fontRegular, color: nero });

    const yTabellaTop = H - MARGINE - logoDims.height - 16;

    let x = MARGINE;
    const disegnaCellaHeader = (label, w) => {
      p.drawRectangle({ x, y: yTabellaTop - headerAltezza, width: w, height: headerAltezza, borderColor: nero, borderWidth: 0.7 });
      p.drawText(label, { x: x + 3, y: yTabellaTop - 14, size: 8, font: fontBold, color: nero, maxWidth: w - 6 });
      x += w;
    };
    colFisse.forEach((c) => disegnaCellaHeader(c.label, c.w));
    persone.forEach((per) => disegnaCellaHeader(nomeCol(per), wPersona));
    disegnaCellaHeader(colFirma.label, colFirma.w);

    return yTabellaTop - headerAltezza;
  }

  let y = disegnaIntestazionePagina(page);
  const righePerPagina = Math.floor((y - MARGINE) / rigaAltezza);
  let righeInPagina = 0;

  for (let i = 0; i < dateLezioni.length; i++) {
    if (righeInPagina >= righePerPagina) {
      page = pdfDoc.addPage([W, H]);
      y = disegnaIntestazionePagina(page);
      righeInPagina = 0;
    }
    const dataLabel = dateLezioni[i].toLocaleDateString("it-IT", { weekday: "short", day: "2-digit", month: "2-digit", year: "numeric" });
    let x = MARGINE;
    const disegnaCella = (testo, w, opz = {}) => {
      page.drawRectangle({ x, y: y - rigaAltezza, width: w, height: rigaAltezza, borderColor: grigio, borderWidth: 0.5 });
      if (testo) page.drawText(String(testo), { x: x + 3, y: y - rigaAltezza + 8, size: opz.size || 8, font: opz.bold ? fontBold : fontRegular, color: nero });
      x += w;
    };
    disegnaCella(String(i + 1) + ".", colFisse[0].w, { bold: true });
    disegnaCella(dataLabel, colFisse[1].w);
    persone.forEach(() => disegnaCella("", wPersona));
    disegnaCella("", colFirma.w);

    y -= rigaAltezza;
    righeInPagina++;
  }

  const bytes = await pdfDoc.save();
  const blob = new Blob([bytes], { type: "application/pdf" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = `Foglio_Presenze_SEDE_${GIORNI_LABEL_FP[turno.giorno_settimana]}_${String(turno.orario || "").slice(0, 5).replace(":", "")}.pdf`;
  document.body.appendChild(a);
  a.click();
  a.remove();
  setTimeout(() => URL.revokeObjectURL(url), 2000);
}
