...
const LOGO_URL = "https://ebsuqdxflygxhuptnnun.supabase.co/storage/v1/object/public/assets/logo.png";
...
export default function SiteHeader() {
  ...
  return (
    <header style={{ background: "#181818", position: "relative", zIndex: 40 }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <a href={SITO} style={{ display: "flex", alignItems: "center" }}>
          <img src={LOGO_URL} alt="A.S.D. Sempre In Forma" style={{ height: 40, width: "auto" }} />
        </a>
          ...
          {VOCI_MENU.map((v) => (
            ...
            >
              ...
              {v.sotto && palestreAperto && (
                <div style={{ position: "absolute", top: "100%", left: 0, background: "#181818", padding: "8px 0", minWidth: 180, borderRadius: 6, boxShadow: "0 8px 20px rgba(0,0,0,.3)" }}>
                  {v.sotto.map((s) => (
      ...
      {menuMobile && (
        <div style={{ background: "#181818", padding: "8px 20px 16px", display: "flex", flexDirection: "column", gap: 4 }}>
          {VOCI_MENU.map((v) => (
